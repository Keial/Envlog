...
function loop(){
  ...
  var cooldown = Settimer30min - nowTime;
  if (cooldown <= 0) {
    ...
    //  cooldown += 3600000;    // cooldown 1 hr.
      ...
      console.log(now);
      console.log(nowTime);
      console.log(Settimer30min);
      console.log(cooldown);
  
          setTimeout(function(){

        dbo_10min_avr.findAll()
  .then((data) => {
  let a1= [data[data.length-1].data1,data[data.length-2].data1,data[data.length-3].data1];
  let a2= [data[data.length-1].data2,data[data.length-2].data2,data[data.length-3].data2];
  let a3= [data[data.length-1].data3,data[data.length-2].data3,data[data.length-3].data3];
